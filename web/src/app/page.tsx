  ...
  return (
    <main className="flex-1 p-4 md:p-8 space-y-8 max-w-7xl w-full mx-auto">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button className="btn-primary w-full md:w-auto shadow-primary/30">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        ...
        <div className="card">
          ...
          <p className="text-3xl md:text-4xl font-black text-gray-900 mt-2">24</p>
        </div>

        <div className="card border-l-4 border-l-success">
          <p className="text-muted font-medium text-sm tracking-wider">ENTREGADOS</p>
          <p className="text-3xl md:text-4xl font-black text-gray-900 mt-2">18</p>
        </div>

        <div className="card border-l-4 border-l-warning">
          <p className="text-muted font-medium text-sm tracking-wider">PENDIENTES</p>
          <p className="text-3xl md:text-4xl font-black text-gray-900 mt-2">6</p>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {[
              ...
              <div key={p.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors cursor-pointer">
                <div className="flex gap-4 items-center">
                  ...
                  <div>
                    <p className="font-bold text-gray-900">{p.cliente}</p>
                    <p className="text-sm text-muted">📍 {p.loc}</p>
                  </div>
                </div>
                <div className="text-right flex flex-col items-end gap-2">
                  <p className="font-bold text-lg">{p.total}</p>
                  <span className={p.estado === 'Entregado' ? 'badge-success' : 'badge-warning'}>
                    {p.estado}
                  </span>
                </div>
              </div>
        ...
        <section className="card space-y-4 border-l-4 border-l-error flex flex-col">
          ...
          <div className="space-y-3 flex-1">
            {[
              { prod: "Garrafa 10kg", stock: 5 },
              { prod: "Carbón", stock: 4 },
              { prod: "Alimento perro", stock: 3 },
            ].map(s => (
              <div key={s.prod} className="flex justify-between items-center p-3 border-b border-gray-100 last:border-0">
                <span className="font-medium text-gray-700">{s.prod}</span>
                <span className="badge-error">{s.stock} un.</span>
              </div>
            ))}
          </div>
          
          <button className="w-full mt-auto btn-secondary">
            Reponer Stock
          </button>
        </section>
      </div>
